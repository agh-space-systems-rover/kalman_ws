// generated from rosidl_generator_c/resource/idl.h.em
// with input from rosbridge_test_msgs:msg/TestHeaderTwo.idl
// generated code does not contain a copyright notice

#ifndef ROSBRIDGE_TEST_MSGS__MSG__TEST_HEADER_TWO_H_
#define ROSBRIDGE_TEST_MSGS__MSG__TEST_HEADER_TWO_H_

#include "rosbridge_test_msgs/msg/detail/test_header_two__struct.h"
#include "rosbridge_test_msgs/msg/detail/test_header_two__functions.h"
#include "rosbridge_test_msgs/msg/detail/test_header_two__type_support.h"

#endif  // ROSBRIDGE_TEST_MSGS__MSG__TEST_HEADER_TWO_H_
